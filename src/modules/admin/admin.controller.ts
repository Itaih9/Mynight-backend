...
export class AdminController {
  ...
  // ---- Promotion email tooling -------------------------------------------
  // The sweep is on a 6h timer and each stage is idempotent, so without these
  // there is no safe way to see or exercise a promotion before it goes out.

  /** Dry run: who WOULD receive an upsell right now. Sends nothing, marks nothing. */
  async previewPromotions(_req: Request, res: Response, next: NextFunction) {
    try {
      const result = await eventsService.runUpsellSweep({ dryRun: true });
      res.json({
        success: true,
        data: {
          wouldSend: result.recipients.length,
          eventsInWindow: result.checked,
  ...
  /** Run the sweep now instead of waiting for the next 6h tick. Really sends. */
  async runPromotions(_req: Request, res: Response, next: NextFunction) {
    try {
      const result = await eventsService.runUpsellSweep();
      res.json({
        success: true,
        data: { sent: result.sent, eventsInWindow: result.checked, recipients: result.recipients },
      });
    } catch (error) {
      next(error);
    }
  ...
  /**
   * Send one promotion email to an arbitrary address with sample data, so the
   * copy and rendering can be checked without touching a real couple.
   */
  async sendTestPromotion(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { to, type = 'upsell', coupleName = 'דנה & יואב', daysUntilWedding = 30 } = req.body || {};
      ...
      if (type === 'welcome') {
        await emailService.sendFlashWelcomeEmail({
          ...
          eventCode: 'TESTCODE',
          weddingDate: new Date(Date.now() + Number(daysUntilWedding) * 86400000),
        });
      } else {
        await emailService.sendFlashUpsellEmail({
          ...
          coupleName: String(coupleName),
          eventCode: 'TESTCODE',
          daysUntilWedding: Number(daysUntilWedding),
        });
      }

      res.json({ success: true, data: { sent: true, to, type } });
    } catch (error) {
      next(error);
    }
  }

  /** Clear an event's upsell history so a stage can be re-tested. */
  async resetPromotionHistory(req: Request, res: Response, next: NextFunction) {
    try {
      await eventsService.resetUpsellHistory(req.params.eventId);
